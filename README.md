...
## Status
Early development, built in phases — see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). Currently **Phase 1 (core SDD-TDD loop)** — the `spec`, `plan`, `implement`, `verify`, and `ship` stages are built, plus an advisory `review` lane and the tag-aware TDD commit hook.

## Requirements
`git` and `node` (the TDD commit hook is cross-platform Node, no shell or `jq`).
