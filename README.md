...
## Status
Early development, built in phases — see [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md). **Phases 1–2 complete**: the core SDD-TDD loop (`spec` → `ship` stages, advisory `review` lane, tag-aware TDD commit hook) plus orchestration — the `drive` orchestrator (`/devloop <feature>`), self-heal, resume-from-artifacts, the `doctor` health check, and the plan-review→re-plan loop. Next: Phase 3 (discuss/research front-end + uncertainty gating).
