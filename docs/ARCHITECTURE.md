...
- **Phase 3 — Front-end + gating.** **In progress** (`feat/phase3-front-end`, local). **Built:** the INTENT/RESEARCH/ASSUMPTIONS artifact contract; **discuss** (triage embedded as phase 1, `auto` autonomous degrade) + `scripts/intent-lint.mjs` (gate exit semantics); **research** (thin skill → `agents/researcher.md`: skill-side fan-out, single-point section-targeted merge, mode-aware greenfield/brownfield, bounded by the ledger, unanswered→named risks) + the stage-gated Q-join lint extension (`stage=research|spec`) + spec's ASSUMPTIONS wiring and terminal Q-join self-check. **Remaining:** mid-pipeline re-gating by blast radius; drive integration of the front-end (incl. `commands/discuss.md` + `.done` markers for discuss/research).
